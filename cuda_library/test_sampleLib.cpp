...
int main() {
  ...
  real *d_var; cuda::memory<real>::allocate(&d_var, N);
  ...
  cuda::blas<real>::setVector(N, h_in, 1, d_var, 1);

  // Launch the kernel
  cuda::display<real>::printVector(d_var, N);

  // Copy the output vector from device to host
  cuda::blas<real>::getVector(N, d_var, 1, h_out, 1);
